...
| 注释智能提取 | 末尾注释处理 | `excel2html_openpyxl_enhanced.py` + `html2chunk.py` |
...
┌─────────────────────────────────────────────────────────────────────────────┐
│  输入: Excel 文件 (.xlsx)                                                    │
│  参数: keywords（可选）, target_tokens（默认1024）, separator                  │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                    ┌─────────────────┴─────────────────┐
                    ▼                                   │
...
#### 5.2.1 `estimate_rows_for_token_limit(html_content, target_tokens=1024)`
...
#### 5.2.2 `run_pipeline(excel_path, keywords=None, max_rows_per_chunk=None, target_tokens=1024, separator="!!!_CHUNK_BREAK_!!!")`
...
# 完整参数示例
python pipeline.py input.xlsx -k "财务" "报表" -t 1024 -s "!!!SPLIT!!!"
```
...
| 目标 Token | `-t` | `--target-tokens` | 1024 | 每个 chunk 的目标 token 数 |
...
📌 第二步：HTML 切分为 Chunks
📊 使用 token 模式，目标每 chunk ≤ 1024 tokens
🔪 切分完成：共生成 15 个片段
...
### 7.6 增强技术 6：注释智能提取与匹配

**问题**: Excel 表格末尾常有注释说明（如"注1：xxx"、"[注] xxx"），这些注释：
- 如果全部添加到每个 chunk，会浪费 token
- 如果完全丢弃，会丢失重要信息

**解决方案**: 智能提取注释，按需分发到相关 chunk。
...
| 类型 | 说明 | 分发策略 |
|------|------|---------|
| `header_notes` | 表头中引用的注释（如表头含 `[注1]`） | 所有 chunk 都添加 |
| `conditional_notes` | 其他注释 | 只添加到包含对应引用的 chunk |

#### 7.6.3 处理流程

**第一阶段（Excel → HTML）**:
1. 检测末尾注释行，解析为 `{key: note}` 格式
2. 检测表头中的注释引用
3. 将注释元数据存入 HTML 的 `<script>` 标签
4. 注释行保留在 `<tbody>` 中，标记 `class="table-note-row"`

**中间结果示例**:
```html
<script type="application/json" class="table-notes-meta">
{
  "header_notes": {"注1": "注1：EX表示从该税号中分出的商品"},
  "conditional_notes": {"注2": "注2：该税率自2026年起执行", "*": "*：暂定税率"}
}
</script>
```
...
**第二阶段（HTML → Chunks）**:
1. 解析注释元数据
2. 过滤掉注释行（`class="table-note-row"`）
3. 每个 chunk 构建时，扫描数据行中的注释引用
4. 只添加匹配的注释到 `rag-context`
...
<div class="rag-context">
  【文档上下文】来源文件：xxx.xlsx | 数据类型：表格数据 
  【表格注释】注1：EX表示从该税号中分出的商品 | 注2：该税率自2026年起执行 | *：暂定税率
</div>
```
...
<div class="rag-context">
  【文档上下文】来源文件：xxx.xlsx | 数据类型：表格数据 
  【表格注释】注1：EX表示从该税号中分出的商品
</div>
```

Chunk 2 不包含 `注2` 和 `*`，因为第3行数据中没有这些引用。

**RAG 效果**:
- 减少无关注释占用的 token
- 保证相关注释不丢失
- 表头引用的注释（如 `[注1]`）所有 chunk 都能看到
